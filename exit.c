...
int nolibc_atexit(void (*func)()) {
  ...
  nolibc_memmove(nolibc_exit_list + 1, nolibc_exit_list, sizeof(struct nolibc_atexit_t) * nolibc_exit_length++);
  nolibc_exit_list[0] = entry;
  return 0;
}
...
int nolibc_at_quick_exit(void (*func)()) {
  ...
  nolibc_memmove(nolibc_at_quick_exit_list + 1, nolibc_at_quick_exit_list, sizeof(struct nolibc_at_quick_exit_t) * nolibc_at_quick_exit_length++);
  nolibc_at_quick_exit_list[0] = entry;
  return 0;
}

__attribute__((noreturn)) void nolibc_exit(int status) {
  if (nolibc_exit_list) {
    for (size_t i = 0; i < nolibc_exit_length; ++i) {
...
__attribute__((noreturn)) void nolibc_quick_exit(int status) {
  if (nolibc_at_quick_exit_list) {
    for (size_t i = 0; i < nolibc_at_quick_exit_length; ++i)
